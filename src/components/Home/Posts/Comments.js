...
export class Comments extends Component {
  ...
  async componentDidMount() {
    const next = await this.props.fetchPostComments(this.props.postId);
      ...
      <div className="comments">
        {loadedComments &&
          ...
          loadedComments.map((comment, index) => {
            return (
              <SingleComment postId={postId} key={index} comment={comment} />
            );
          })}
        {loadingComments ? <LoadingCommentsPlaceholder /> : null}
        {loadedComments.length === 0 && !loadingComments && (
          <h4 style={{ textAlign: "center", color: "#606770" }}>No Comments</h4>
        )}
        <div className="post-comment">
          ...
          <CommentInput
            postId={postId}
            addCommentLocally={(message, commentId) =>
              this.addCommentLocally(message, commentId)
            }
          />
        </div>
      </div>
