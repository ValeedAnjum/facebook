...
class Posts extends Component {
  ...
  async componentDidMount() {
    const next = await this.props.fetchPost();
  ...
  loadNextPost = async () => {
    ...
    const next = await this.props.fetchPost(lastPostId);
    if (next && next.docs && next.docs.length >= 1) {
      this.setState({
        loadedPosts: [...this.state.loadedPosts, ...this.props.post],
      });
    }
  ...
  render() {
    ...
    const { photoUrl, likePost, unlikePost } = this.props;
    return (
